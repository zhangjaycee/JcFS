...
{
#ifdef DEBUG_JC
    FILE *log_fp = fopen("/root/JcFS/log.txt", "a+");
#endif
    int count = 0;
    size_t copied = 0;
...
#ifdef DEBUG_JC
        count += 1;
        fprintf(log_fp, "count: %d,  src_len: %zu,  dst_len: %zu, copied: %zu\n", count, src_len, dst_len, copied);
#endif
...
#ifdef DEBUG_JC
    count += 1;
    fprintf(log_fp, "count: %d,  copied: %zu\n", count, copied);
    fclose(log_fp);
#endif
